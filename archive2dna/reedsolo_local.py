...
def find_prime_polys(generator=2, c_exp=8, fast_primes=False, single=False):
    ...
    if fast_primes:
        ...
        prim_candidates = [
            x for x in prim_candidates if x > field_charac
        ]  # filter out too small primes
    else:
        prim_candidates = xrange(
...
def rs_encode_msg(msg_in, nsym, fcr=0, generator=2, gen=None):
    ...
    msg_out[
        : len(msg_in)
    ] = msg_in  # equivalent to c = mprime - b, where mprime is msg_in padded with [0]*nsym
    return msg_out
...
def rs_correct_errata(
    msg_in, synd, err_pos, fcr=0, generator=2
...
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    err_loc = list(
        itertools.dropwhile(lambda x: x == 0, err_loc)
...
def rs_find_errors(err_loc, nmess, generator=2):
    ...
    for i in xrange(
        ...
        if (
            gf_poly_eval(err_loc, gf_pow(generator, i)) == 0
...
def rs_correct_msg(
    ...
    return (
        ...
        msg_out[-nsym:],
        erase_pos + err_pos,
    )  # also return the corrected ecc block so that the user can check(), and the position of errors to allow for adaptive bitrate algorithm to check how the number of errors vary
...
class RSCodec(object):
    ...
    def __init__(
        ...
        if (
            nsize > 255 and c_exp <= 8
        ):  # nsize (chunksize) is larger than the galois field, we resize the galois field
        ...
        self.fcr = (
            fcr  # first consecutive root, can be any value between 0 and (2**c_exp)-1
        )
    ...
    def maxerrata(self, errors=None, erasures=None, verbose=False):
        ...
        maxerrors = int(
            nsym / 2
        )  # always floor the number, we can't correct half a symbol, it's all or nothing
        maxerasures = nsym
