...
def process_root():
    if request.method == 'GET':
        ...
        u = us[0] + '://' + us[2] + '/'
        response['description'] = 'Encodes binary file to DNA and decodes back from DNA to binary.'
        response['routes'] = {'encode': {'route': u + 'encode',
                                         'description' : 'Encodes binary file to DNA.'}, 
                              'decode': {'route': u + 'decode',
...
def process_encode():
    if request.method == 'GET':
        ...
        response['request-parameters'] = {'data': 'binary file', 'id' : 'information package id [optionnal]'}
        response['example'] = 'curl -F id="test:1" -F data="@aip.zip" -X POST http://localhost:8080/encode'
...
def process_decode():
    if request.method == 'GET':
        ...
        response = response_template
        response['description'] = 'Decodes DNA text file to binary file. DNA text file consists of one DNA segment per line (composed of letters A, T, G and C) and is encoded in utf-8.'
        response['request-verb'] = 'POST'
        response['request-parameters'] = {'data': 'dna file', 'id' : 'information package id [optionnal]'}
        response['example'] = 'curl --data-biinary @dna.txt -X POST http://localhost:8080/decode -o aip_decoded.zip'
        return jsonify(response)
